...
def analyze(holdings: list[Holding], settings: Settings) -> dict:
    ...
    for h in holdings:
        try:
            ...
            market_value = h.shares * price.price
    ...
    currencies = {r.currency for r in valid}
    return {
        "rows": rows,
...
def build_markdown(result: dict) -> str:
    lines = ["# Performance del portafoglio", ""]
    tot_pl = result["total_pl_pct"]
    lines.append(
        f"**Totale:** carico {result['total_cost']} → mercato {result['total_market']} "
        f"(**{tot_pl:+.2f}%**, {result['total_pl_abs']:+.2f})"
        if tot_pl is not None else "**Totale:** dati insufficienti."
    )
    if result["mixed_currency"]:
        lines.append(
            f"\n> ⚠️ Valute miste ({', '.join(result['currencies'])}): i totali sono "
            "una somma nominale senza conversione di cambio."
        )
    lines.append("")
    lines.append("| Titolo | Valuta | Carico | Mercato | P/L | P/L % | Peso % |")
    lines.append("|---|---|---|---|---|---|---|")

...
def test_render_contains_key_sections(monkeypatch):
    ...
    # equity curve: due punti -> il grafico SVG deve comparire
    monkeypatch.setattr(history, "load", lambda *a, **k: [
        {"date": "2026-01-01", "total_market": 900.0},
        {"date": "2026-01-02", "total_market": 1000.0},
    ])
    ...
    assert "Andamento patrimonio" in htmltext and "<polyline" in htmltext  # equity curve
    print("✓ render dashboard ok (KPI, equity curve, alert, no info, eventi)")

...
def test_mixed_currency_flag(monkeypatch):
    prices = {"AAA": PriceSnapshot(ticker="AAA", price=200.0, currency="EUR"),
              "USD1": PriceSnapshot(ticker="USD1", price=200.0, currency="USD")}
    monkeypatch.setattr(providers, "get_price", lambda t: prices[t])
    holdings = [Holding("AAA", 1, 100), Holding("USD1", 1, 100)]
    r = returns.analyze(holdings, Settings.load(require_llm=False))
    assert r["mixed_currency"] is True and r["currencies"] == ["EUR", "USD"]
    print("✓ flag valute miste ok")
